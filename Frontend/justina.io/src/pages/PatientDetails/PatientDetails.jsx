...
const PatientDetails = () => {
  // TODO: Constante sacar del componente para evitar re-crear el objeto en re-render
  const doctors = [
  ...
  // TODO: Component in Component, sacar fuera se vuelve a crear en cada re-render
  const renderDoctorCollapsed = (doctor) => (
    <div>
      <img
        src={doctor.photo}
        alt={doctor.name}
        className="w-10 h-10 rounded-full hover:shadow-custom"
      />
    </div>
  );
  // TODO: Component in Component, sacar fuera se vuelve a crear en cada re-render
  const renderDoctorExpanded = (doctor) => (
    <div
      ...
      className="flex items-center group backdrop-blur bg-[rgba(253,239,244,0.1)] rounded-3xl p-2 cursor-pointer"
    >
      <img
        src={doctor.photo}
        alt={doctor.name}
      ...
      <div className="flex items-center flex-grow justify-end group-hover:justify-center pl-2">
        <span className="transition-transform duration-300 ease-in-out group-hover:translate-x-[-20%]">
          {doctor.name}
        </span>
        <button className="flex items-center justify-center w-6 h-6 rounded-full ml-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <img src={chevronDown} />
        </button>
      </div>
    </div>
  ...
  return (
    <>
      <section className="px-6 flex gap-6 w-full">
        <button className="flex flex-col items-center bg-[rgba(214,86,131,0.2)] backdrop-blur rounded-3xl py-1 px-10 mb-4 shadow-custom text-neutrals800 text-titulopag font-bold">
          Prioridad{" "}
          <img
            src={priorityIcon}
            className="bg-[rgba(253,239,244,.4)] py-1 px-2 rounded-full w-11 h-8"
          />
          <span className="text-subtitulo">Alta</span>
        </button>
      ...
      <section className="flex gap-4 p-3 pl-6 w-full">
        ...
        <div className="flex flex-col items-start gap-4 w-2/5">
          <h3 className="text-primary text-parrafo">
            Gráfico de Signos Vitales
          </h3>
          <img src={VitalSigns} className="w-full" />
          <h3 className="text-primary text-parrafo pt-2">
        ...
        <div className="w-1/3 flex flex-col gap-4">
          ...
          <ShortcutBtn
            ...
            to="/medicalrecord"
          />
          <ShortcutBtn title="Exámenes Médicos" icon={pillIcon} />
          <ShortcutBtn
            title="Tratamientos"
            icon={pillIcon}
            to="/treatmentform"
          />
          <ShortcutBtn title="Prepagas" icon={pillIcon} />
          <ExpandableList
            items={doctors}
